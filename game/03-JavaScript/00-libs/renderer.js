...
(function (Renderer) {
	...
	function createGradient(spec) {
		...
		switch (spec.gradient) {
			...
			case "radial":
				...
				break;
			default:
				throw new Error("잘못된 그라데이션 타입: " + spec.gradient);
		}
		if (spec.colors.length < 2) throw new Error("잘못된 그라디언트 색상 지점: " + JSON.stringify(spec.colors));
		...
		for (let k of Object.keys(source)) {
			if (k === "brightness" && "brightness" in target) {
				if (typeof target.brightness === "object" && typeof source.brightness === "number") {
					...
					throw new Error("미구현: 두 그라디언트 밝기값은 병합할 수 없습니다.");
				}
	...
	function adjustGradientBrightness(image, frameCount, brightness, resultCanvas) {
		if (brightness.adjustments.length !== 2) {
			throw new Error("미구현: 밝기 그라디언트는 정확히 2개의 지점만 가질 수 있습니다.");
		}
	...
	function composeLayers(targetCanvas, layerSpecs, frameCount, listener) {
		...
		const layers = layerSpecs
			...
			.map((layer, i) => {
				if (isNaN(layer.z)) {
					console.error("레이어 " + (layer.name || layer.src) + "의 z-index가 NaN입니다");
					layer.z = 0;
				}
				return [layer, i];
			...
			Renderer.ImageLoader.loadImage(
				...
				(src, layer, error) => {
					...
					if (listener && listener.loadError) {
						...
						console.error("이미지 로드 실패: " + src + (layer.name ? " (레이어: " + layer.name + ")" : ""));
					}
					layer.show = false;
					maybeRenderResult();
				}
		...
		function loadLayerMask(layer) {
			...
			layer.masksrc.forEach((src, index) => {
				...
				Renderer.ImageLoader.loadImage(
					...
					(src, layer, error) => {
						console.error("마스크 로드 실패: " + src + (layer.name ? " (레이어: " + layer.name + ")" : ""));
						layer.show = false;
						Renderer.ImageErrors[src] = true;
						layer.masksrc = null;
						maybeRenderResult();
